...
from core.database import create_all_tables, get_or_create_worker, get_worker_profile, get_pool
...
async def get_session_state(phone_number: str):
    ...
    return {
        ...
        "progress": {
            ...
            "total": 20,
            "percent": int((session["onboarding"].get("questions_answered", 0) / 20) * 100)
        },
...
async def get_resume_url(phone_number: str):
    ...
    if not profile or not profile.get("resume_s3_key"):
        raise HTTPException(status_code=404, detail="Resume not found. Complete your profile first.")
